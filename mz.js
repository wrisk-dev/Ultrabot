...
// ===== Quote (Block Quote) Helpers - all bot messages quoted =====
...
async function qReply(ctx, text, extra = undefined) {
  try {
    if (extra) {
      return await ctx.reply(applyQuote(text), { parse_mode: 'HTML', ...extra });
    }
    return await ctx.reply(applyQuote(text), { parse_mode: 'HTML' });
  } catch (error) {
...
// MARRY STRATEGIES
// =========================================================
// The source list is normalized to newest-first before applying each formula.
function calculateFormula(list) {
    if (list.length < 2) return "Big";

    let num1 = parseInt(list[0].number || list[0].openNum || list[0].result || 0);
    let num2 = parseInt(list[1].number || list[1].openNum || list[1].result || 0);
...
function getMarryPrediction(strategy, userId) {
    const numbers = (userLastNumbers[userId] || [])
        .slice(-2)
        .reverse()
        .map(number => ({ number }));

    if (strategy === "MARRY_V1") {
        return calculateFormula(numbers) === "Big" ? "B" : "S";
    }

    if (numbers.length === 0) return "B";
    const num1 = parseInt(numbers[0].number || 0) % 10;
    const num2 = parseInt(numbers[1]?.number || 0) % 10;

    if (strategy === "MARRY_V2") {
        // Latest number parity: even => Big, odd => Small.
        return num1 % 2 === 0 ? "B" : "S";
    }
    if (strategy === "MARRY_V3") {
        // Latest two-number average: average >= 5 => Big, otherwise Small.
        return ((num1 + num2) / (numbers.length === 2 ? 2 : 1)) >= 5 ? "B" : "S";
    }
    if (strategy === "MARRY_V4") {
        // Latest two-number distance parity: even => Big, odd => Small.
        return Math.abs(num1 - num2) % 2 === 0 ? "B" : "S";
    }
    return "B";
}
...
// ===== ALL Buttons Colored (green/blue/red rotation, no emoji needed) =====
const BUTTON_COLORS = ['success', 'primary', 'danger']; // green, blue, red
let __btnColorIdx = 0;
const __origBtnCallback = Markup.button.callback;
const __origBtnUrl = Markup.button.url;
Markup.button.callback = function(text, data, hide) {
  const btn = __origBtnCallback.call(this, text, data, hide);
  btn.style = BUTTON_COLORS[__btnColorIdx++ % BUTTON_COLORS.length];
  return btn;
};
Markup.button.url = function(text, url, hide) {
  const btn = __origBtnUrl.call(this, text, url, hide);
  btn.style = BUTTON_COLORS[__btnColorIdx++ % BUTTON_COLORS.length];
  return btn;
};

// ===== Reply Keyboard (menu) buttons colored too (Bot API 9.4: style works on reply keyboards) =====
const __origMarkupKeyboard = Markup.keyboard;
Markup.keyboard = function(buttons, options) {
  const styled = buttons.map(row => row.map(btn => {
    if (typeof btn === 'string') {
      return { text: btn, style: BUTTON_COLORS[__btnColorIdx++ % BUTTON_COLORS.length] };
    }
    if (btn && typeof btn === 'object' && btn.text && !btn.style) {
      return { ...btn, style: BUTTON_COLORS[__btnColorIdx++ % BUTTON_COLORS.length] };
    }
    return btn;
  }));
  return __origMarkupKeyboard.call(this, styled, options);
};
// ===== END Colored Buttons =====
...
const userAIPredictionCache = {}; // AI Predictor (Manual Bet): 1 period = 1 prediction
...
// ===== AI PREDICTOR (Manual Bet) - Deterministic Random BS =====
// Same site + same game + same period => same prediction on ALL devices
// 1 Period = 1 Prediction. Prediction never changes until period changes.
function getDeterministicAIPrediction(period, site, gameType) {
  const seed = `${site}|${gameType}|${String(period)}`;
  const hash = crypto.createHash('md5').update(seed).digest('hex');
  const num = parseInt(hash.substring(0, 8), 16);
  return num % 2 === 0 ? 'B' : 'S';
}

function makeAIPredictorKeyboard(prediction) {
  const label = prediction === 'B' ? 'BIG' : 'SMALL';
  return Markup.inlineKeyboard([
    [{ text: "AI PREDICTOR", callback_data: "ai_predict:refresh", style: "success" }],
    [{ text: `PREDICTION: ${label}`, callback_data: "ai_predict:show", style: "primary" }],
    [{ text: "CLOSE", callback_data: "ai_predict:close", style: "danger" }]
  ]);
}

async function handleAIPredictorCommand(ctx, userId, editMessageId = null) {
  try {
    ...
    const site = userSelectedSite[userId] || DEFAULT_SITE;

    const issueRes = await getGameIssueRequest(session, gameType);
    if (!issueRes || issueRes.code !== 0) {
      await sendMessageWithRetry(ctx, "Failed to get current period. Try again.");
      return;
    }
    ...
    if (gameType === "TRX") {
      currentIssue = data.predraw?.issueNumber || data.predraw?.issue || null;
    } else {
      currentIssue = data.issueNumber || null;
    }

    if (!currentIssue) {
      await sendMessageWithRetry(ctx, "Failed to get period number. Try again.");
      return;
    }

    // 1 Period 1 Prediction: cached per period, never changes until period changes
    let prediction;
    const cache = userAIPredictionCache[userId];
    if (cache && cache.period === currentIssue && cache.site === site && cache.gameType === gameType) {
      prediction = cache.prediction;
      logging.info(`AI Predictor: cached prediction for period ${currentIssue} -> ${prediction}`);
    } else {
      prediction = getDeterministicAIPrediction(currentIssue, site, gameType);
      userAIPredictionCache[userId] = { period: currentIssue, prediction, site, gameType };
      logging.info(`AI Predictor: new prediction for period ${currentIssue} -> ${prediction}`);
    }
    ...
    const keyboard = makeAIPredictorKeyboard(prediction);

    if (editMessageId) {
      try {
        await ctx.telegram.editMessageText(userId, editMessageId, undefined, message, { reply_markup: keyboard.reply_markup });
        return;
      } catch (error) {
        logging.error(`AI Predictor edit error: ${error.message}`);
      }
    }
    await sendMessageWithRetry(ctx, message, keyboard);
  } catch (error) {
    logging.error(`AI Predictor error: ${error.message}`);
    await sendMessageWithRetry(ctx, `AI Predictor error: ${error.message}`);
  }
}
// ===== END AI PREDICTOR =====
...
async function sendMessageWithRetry(ctx, text, replyMarkup = null) {
  for (let attempt = 0; attempt < MAX_TELEGRAM_RETRIES; attempt++) {
    try {
      if (replyMarkup) {
        try {
          await qReply(ctx, applyQuote(text), { parse_mode: 'HTML', ...replyMarkup });
        } catch (quoteError) {
          await qReply(ctx, text, replyMarkup);
        }
      } else {
        try {
          await qReply(ctx, applyQuote(text), { parse_mode: 'HTML' });
        } catch (quoteError) {
          await qReply(ctx, text);
        }
      }
      return true;
    } catch (error) {
...
function getCurrentYangonTime() {
  return moment().tz('Asia/Yangon').format('YYYY-MM-DD HH:mm:ss');
      ...
      } else if (["MARRY_V1", "MARRY_V2", "MARRY_V3", "MARRY_V4"].includes(settings.strategy)) {
        ch = getMarryPrediction(settings.strategy, userId);
        logging.info(`${settings.strategy}: Prediction is ${ch === 'B' ? 'Big' : 'Small'}`);
...
function makeStrategyKeyboard(userId = null) {
  ...
  if (gameType === "TRX") {
    keyboard = [
      ...
      [
        Markup.button.callback("💍 MARRY V1", "strategy:MARRY_V1"),
        Markup.button.callback("💍 MARRY V2", "strategy:MARRY_V2")
      ],
      [
        Markup.button.callback("💍 MARRY V3", "strategy:MARRY_V3"),
        Markup.button.callback("💍 MARRY V4", "strategy:MARRY_V4")
      ]
    ];
  } else {
    keyboard = [
...
async function cmdAdminPanelHandler(ctx) {
  ...
  try {
    await qReply(ctx, welcomeMessage, { 
  ...
  if (imageBuffer) {
    await ctx.replyWithPhoto(
      { source: imageBuffer },
      {
        ...
        await qReply(ctx, message, { parse_mode: 'HTML' });
      }
    } else {
...
async function cmdStartHandler(ctx) {
  ...
  try {
    ...
    if (photos.total_count > 0) {
      ...
      const welcomeKeyboard = Markup.inlineKeyboard([
        ...
        [Markup.button.url("📢 Join Updates Channel", "https://t.me/Yoonngel2272/1819")]
      ]);
      
      const welcomeCaption = 
        `🤖 LOTTERY AUTOBET VERSION 1.5\n\n` +
        `Welcome, ${firstName}! 👋\n\n` +
      ...
      await qReply(ctx, welcomeMessage, welcomeKeyboard);
    }
  } catch (error) {
  ...
  if (!loggedIn) {
    await qReply(ctx, "Please login to continue:", makeMainMenuKeyboard(false, userId));
  } else {
    if (userId === ADMIN_ID) {
      await qReply(ctx, "Welcome back, Admin!", makeMainMenuKeyboard(true, userId));
    } else {
      await qReply(ctx, "Welcome back!", makeMainMenuKeyboard(true, userId));
    }
  }
}
...
async function tutorialCallbackHandler(ctx) {
  try {
    ...
    await qReply(ctx, "☃️ Here's the tutorial video");
...
async function cmdAllowHandler(ctx) {
  ...
  await qReply(ctx, "Admin Panel:", makeAdminPanelKeyboard());
}
...
if (data.startsWith("reactivate:")) {
  ...
  // AI Predictor buttons (colored: green/blue/red, no emoji)
  if (data.startsWith("ai_predict:")) {
    const action = data.split(":")[1];

    if (action === "refresh") {
      const msgId = ctx.callbackQuery?.message?.message_id;
      await handleAIPredictorCommand(ctx, userId, msgId);
      return;
    }

    if (action === "show") {
      const cache = userAIPredictionCache[userId];
      const label = cache ? (cache.prediction === 'B' ? 'BIG' : 'SMALL') : 'N/A';
      try { await ctx.answerCbQuery(`Prediction: ${label}`); } catch (e) {}
      return;
    }
...
async function showUserStats(ctx, userId) {
  try {
    ...
    else if (strategyDisplay === 'MARRY_V1') strategyDisplay = 'MARRY V1';
    else if (strategyDisplay === 'MARRY_V2') strategyDisplay = 'MARRY V2';
    else if (strategyDisplay === 'MARRY_V3') strategyDisplay = 'MARRY V3';
    else if (strategyDisplay === 'MARRY_V4') strategyDisplay = 'MARRY V4';
...
async function textMessageHandler(ctx) {
  ...
  // === ADMIN PANEL BUTTONS HANDLING (fix: these were never wired) ===
  if (userId === ADMIN_ID) {
    const adminPanelButtons = [
      "👤 Add User", "🗑️ Remove User", "📊 User Stats", "📋 Allowed IDs",
      "✅ Enable Free Mode", "❌ Disable Free Mode", "📢 Broadcast", "🔍 Check Free Mode"
    ];
    const matchedAdminButton = adminPanelButtons.find(btn => rawText.includes(btn));
    if (matchedAdminButton) {
      await handleAdminPanelCommand(ctx, userId, userName, matchedAdminButton);
      return;
    }
  }
  ...
  // === AI PREDICTOR (Manual Bet) ===
  if (rawText.includes("AI Predictor") || rawText.toLowerCase().includes("ai predictor")) {
    await handleAIPredictorCommand(ctx, userId);
    return;
  }
...
// ===== Quote all direct telegram sendMessage calls =====
const __origTgSendMessage = bot.telegram.sendMessage.bind(bot.telegram);
bot.telegram.sendMessage = async (chatId, text, extra) => {
  try {
    return await __origTgSendMessage(chatId, applyQuote(text), { parse_mode: 'HTML', ...(extra || {}) });
  } catch (error) {
    return await __origTgSendMessage(chatId, text, extra);
  }
};
// ===== END =====
