...
async function bettingWorker(userId, ctx, bot) {
  ...
  try {
    while (settings.running) {
      ...
      // RISK AURA always places a prediction, even when remote history is unavailable.
      // Its predictor already falls back to the local last-10 results/default B.
      if (settings.strategy === "RISK_AURA") {
        if (shouldSkip) {
          logging.warning(`RISK_AURA: overriding skip (${skipReason || 'unspecified'}) and placing the fallback prediction`);
        }
        shouldSkip = false;
        skipReason = "";
      }
...
${EMOJI.INFO} Enter a mode number:`,
      ...
      await sendMessageWithRetry(ctx, `${EMOJI.SUCCESS} ${STYLE.BOLD(`${strategyLabel} Activated`)}`,
        makeMainKeyboard(true, userId === ADMIN_ID));
    } else if (["RISK_AURA", "KIZON_V2"].includes(strategy)) {
      const strategyLabel = strategy === "RISK_AURA" ? "RISK AURA" : "KIZON V2";
