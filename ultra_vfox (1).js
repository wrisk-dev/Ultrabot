...
// RISK AURA: use the latest result digit to locate a matching issue suffix,
// multiply the two result digits, then reduce the product to a single digit.
function getRiskAuraPrediction(outcomes) {
  const history = (outcomes || []).slice(-500);
  if (!history.length) {
    return { choice: 'B', shouldSkip: true, skipReason: 'RISK AURA requires recent results' };
  }

  const latest = history[history.length - 1];
  const latestNum = Number.parseInt(latest.number, 10);
  if (!Number.isFinite(latestNum)) {
    return { choice: 'B', shouldSkip: true, skipReason: 'RISK AURA received invalid result data' };
  }
...
async function bettingWorker(userId, ctx, bot) {
  ...
  try {
    while (settings.running) {
      ...
      if (["R_PAT", "CHI_SQUARE_ANOMALY_DETECTION", "MULTIPLICATION_MATRIX", "KIZON", "RISK_AURA", "KIZON_V2"].includes(settings.strategy)) {
      ...
      if (settings.strategy === "KIZON") {
        ...
        const prediction = getRiskAuraPrediction(strategyOutcomes);
        ch = prediction.choice;
        shouldSkip = prediction.shouldSkip;
