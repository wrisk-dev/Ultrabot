...
async function createLiveStatsImage(startAmount, profit, balance, isVirtual, gameType, strategy, betWrager, userId) {
  ...
  return new Promise((resolve, reject) => {
    out.once('finish', () => resolve(imagePath));
...
async function checkProfitAndStopLoss(userId, bot) {
  ...
  if (targetProfit && currentProfit >= targetProfit) {
    ...
    try {
      ...
      await bot.telegram.sendPhoto(userId, { source: fs.createReadStream(imagePath) }, {
...
// Dedicated RISK AURA fetch: request only the latest 10 TRX draws.
async function getRiskAuraRecent10(session) {
  const body = {
    pageNo: 1,
    pageSize: 10,
    language: 0,
  ...
  try {
    ...
    const data = response.data || {};
    const rawList = data?.data?.list || data?.data?.data?.gameslist || [];
    const success = data?.msgCode === 0 || data?.code === 0;
    if (!success || !Array.isArray(rawList)) {
      logging.warning(`RISK_AURA: 10-result API failed: ${data?.msg || data?.msgCode || data?.code || 'No data'}`);
      return [];
    }
    ...
    const history = rawList
      ...
      .filter(item => item.issueNumber && Number.isFinite(item.number))
      .slice(0, 10)
      .reverse();
    logging.info(`RISK_AURA: fetched ${history.length}/10 TRX results from GetTRXNoaverageEmerdList`);
    return history;
  } catch (error) {
    logging.error(`RISK_AURA: 10-result API request error: ${error.message}`);
    return [];
  }
}
...
// RISK AURA: use the latest result digit from the latest 10 results to locate
// a matching issue suffix, multiply the two result digits, then reduce the
// product to a single digit.
function getRiskAuraPrediction(outcomes, fallbackNumbers = []) {
  const remoteHistory = (outcomes || []).slice(-10);
  const history = remoteHistory.length > 0
    ? remoteHistory
    : fallbackNumbers.slice(-10).map(number => ({ number, issueNumber: '' }));
  if (!history.length) {
    return { choice: 'B', shouldSkip: false, skipReason: 'RISK AURA fallback default' };
  }

  const latest = history[history.length - 1];
  const latestNum = Number.parseInt(latest.number, 10);
  if (!Number.isFinite(latestNum)) {
    return { choice: 'B', shouldSkip: false, skipReason: 'RISK AURA invalid data fallback' };
  }

  const targetDigit = Math.abs(latestNum) % 10;
  const previous = history.slice(0, -1);
  const matched = [...previous].reverse().find(item => {
    const issue = String(item.issueNumber ?? '');
    return /\d$/.test(issue) && Number(issue.slice(-1)) === targetDigit;
  });
  const foundNum = matched ? Number.parseInt(matched.number, 10) : latestNum;
  const product = latestNum * (Number.isFinite(foundNum) ? foundNum : latestNum);
  let digitSum = Math.abs(product);
  while (digitSum >= 10) {
    digitSum = String(digitSum).split('').reduce((sum, digit) => sum + Number(digit), 0);
  }
  const choice = digitSum >= 5 ? 'B' : 'S';
  logging.info(`RISK_AURA: latest=${latestNum}, target_digit=${targetDigit}, found=${foundNum}, product=${product}, digit=${digitSum}, prediction=${choice}`);
  return { choice, shouldSkip: false, latestNum, foundNum, product, digit: digitSum };
}

function getRiskAuraV2Prediction(outcomes, fallbackNumbers = []) {
  const prediction = getRiskAuraPrediction(outcomes, fallbackNumbers);
  const choice = prediction.choice === 'B' ? 'S' : 'B';
  logging.info(`RISK_AURA_V2: base=${prediction.choice}, inverse=${choice}`);
  return { ...prediction, choice, shouldSkip: false };
}

function getRiskAgentPrediction(outcomes, fallbackNumbers = []) {
  const remoteHistory = (outcomes || []).slice(-10).reverse();
  const history = remoteHistory.length > 0
    ? remoteHistory
    : fallbackNumbers.slice(-10).reverse().map(number => ({ number }));
  const typedHistory = history
    .map(item => {
      const number = Number.parseInt(item.number, 10);
      return { number, actualType: number >= 0 && number <= 4 ? 'SMALL' : 'BIG' };
    })
    .filter(item => Number.isFinite(item.number));

  if (!typedHistory.length) {
    const signal = Math.random() > 0.5 ? 'BIG' : 'SMALL';
    const confidence = (65 + Math.random() * 10).toFixed(1);
    logging.info(`RISK_AGENT: no history, signal=${signal}, confidence=${confidence}%`);
    return { choice: signal === 'BIG' ? 'B' : 'S', signal, confidence, shouldSkip: false };
  }

  const recent = typedHistory.slice(0, 5);
  let bigWeight = 0;
  let smallWeight = 0;
  recent.forEach((item, index) => {
    const weight = 5 - index;
    if (item.actualType === 'BIG') bigWeight += weight;
  ...
  if (bigWeight > smallWeight) {
    signal = 'BIG';
    rawScore = (bigWeight / (bigWeight + smallWeight)) * 100;
  } else if (smallWeight > bigWeight) {
    signal = 'SMALL';
    rawScore = (smallWeight / (bigWeight + smallWeight)) * 100;
  } else {
    signal = recent[0].actualType === 'BIG' ? 'SMALL' : 'BIG';
    rawScore = 60.0;
  }
  const confidence = Math.min(85.0, Math.max(65.0, rawScore)).toFixed(1);
  const choice = signal === 'BIG' ? 'B' : 'S';
  logging.info(`RISK_AGENT: recent=${recent.map(item => item.actualType[0]).join('')}, bigWeight=${bigWeight}, smallWeight=${smallWeight}, signal=${signal}, confidence=${confidence}%`);
  return { choice, signal, confidence, bigWeight, smallWeight, shouldSkip: false };
}

// KIZON V2: 500-result engine based on a three-result streak breaker,
// five-result average momentum, and newest-result parity fallback.
function getKizonV2Prediction(outcomes) {
  const newestFirst = (outcomes || []).slice(-500).reverse();
  const numbers = newestFirst
    .map(item => Number.parseInt(item.number, 10))
    .filter(Number.isFinite);
  if (numbers.length < 5) {
    return { choice: 'S', shouldSkip: true, skipReason: 'KIZON V2 requires at least 5 results' };
  }

  const types = numbers.map(number => number >= 5 ? 'B' : 'S');
  let logic;
  let choice;
  const avgNum = numbers.slice(0, 5).reduce((sum, number) => sum + number, 0) / 5;
  if (types[0] === types[1] && types[1] === types[2]) {
    choice = types[0] === 'B' ? 'S' : 'B';
    logic = '6_STREAK_BREAKER';
  } else if (avgNum >= 4.8) {
    choice = 'B';
    logic = '6_HIGH_MOMENTUM';
  } else {
    choice = numbers[0] % 2 === 0 ? 'S' : 'B';
    logic = '6_PARITY_SHIFT';
  }
  logging.info(`KIZON_V2: latest5=${numbers.slice(0, 5).join(',')}, avg=${avgNum.toFixed(2)}, logic=${logic}, prediction=${choice}`);
  return { choice, shouldSkip: false, confidence: 96.1, logic, average: avgNum };
}

// KIZON Strategy: 20 selectable prediction modes from the supplied logic.
// The API cache is chronological (oldest -> newest), while the original
// KIZON rules expect newest-first arrays.
function getKizonPrediction(outcomes, mode = 20) {
  const newestFirst = (outcomes || []).slice(-20).reverse();
  const numbers = newestFirst.map(item => Number(item.number)).filter(Number.isFinite);
  const types = numbers.map(number => number >= 5 ? 'BIG' : 'SMALL');
  const selectedMode = Number.isInteger(Number(mode)) && Number(mode) >= 1 && Number(mode) <= 20
    ? Number(mode) : 20;

  if (numbers.length === 0) {
    return { choice: 'B', shouldSkip: true, skipReason: 'KIZON requires recent results', mode: selectedMode };
  }

  const n = (index, fallback = 0) => numbers[index] ?? fallback;
  const t = (index, fallback = 'SMALL') => types[index] ?? fallback;
  let prediction;

  switch (selectedMode) {
    case 1: prediction = numbers.slice(0, 5).reduce((a, b) => a + b, 0) >= 22 ? 'BIG' : 'SMALL'; break;
    case 2: prediction = types.slice(0, 5).filter(v => v === 'BIG').length >= 3 ? 'BIG' : 'SMALL'; break;
    case 3: prediction = n(0) % 2 === 0 ? 'SMALL' : 'BIG'; break;
    case 4: prediction = types.slice(0, 2).filter(v => v === t(0)).length === 2 ? t(0) : (t(1) === 'SMALL' ? 'BIG' : 'SMALL'); break;
    case 5: prediction = t(0) === 'BIG' ? 'SMALL' : 'BIG'; break;
    case 6: prediction = numbers.slice(0, 3).reduce((a, b) => a + b, 0) > 13 ? 'BIG' : 'SMALL'; break;
    case 7: prediction = t(2, 'BIG'); break;
    case 8: prediction = n(0) >= 5 ? 'BIG' : 'SMALL'; break;
    case 9: prediction = types.slice(0, 3).filter(v => v === 'SMALL').length >= 2 ? 'SMALL' : 'BIG'; break;
    case 10: prediction = (n(0) + n(1)) % 2 === 0 ? 'BIG' : 'SMALL'; break;
    case 11: prediction = numbers.slice(0, 4).reduce((a, b) => a + b, 0) < 20 ? 'SMALL' : 'BIG'; break;
    case 12: prediction = t(0) === 'SMALL' ? 'BIG' : 'SMALL'; break;
    case 13: prediction = [0, 1, 2, 3, 4].includes(n(0)) ? 'SMALL' : 'BIG'; break;
    case 14: prediction = numbers.length > 1 && t(0) !== t(1) ? 'BIG' : 'SMALL'; break;
    case 15: prediction = (n(0) + n(1)) % 2 !== 0 ? 'SMALL' : 'BIG'; break;
    case 16: prediction = types.slice(0, 4).filter(v => v === 'BIG').length >= 2 ? 'BIG' : 'SMALL'; break;
    case 17: prediction = n(0) < 5 ? 'SMALL' : 'BIG'; break;
    case 18: prediction = numbers.slice(0, 3).reduce((a, b) => a + b, 0) % 2 === 0 ? 'BIG' : 'SMALL'; break;
    case 19: prediction = t(0) === 'BIG' && t(1) === 'SMALL' ? 'SMALL' : 'BIG'; break;
    case 20: default: prediction = n(0) % 2 !== 0 ? 'BIG' : 'SMALL'; break;
  }

  const choice = prediction === 'BIG' ? 'B' : 'S';
  logging.info(`KIZON mode ${selectedMode}: numbers=${numbers.slice(0, 5).join(',')}, types=${types.slice(0, 5).join(',')}, prediction=${prediction}`);
  return { choice, shouldSkip: false, prediction, mode: selectedMode };
}

function autoFallbackKizon(settings, isWin) {
  if (!settings || settings.strategy !== "KIZON") return;

  if (isWin) {
    settings.kizon_consecutive_losses = 0;
    logging.info(`KIZON: win detected; consecutive_losses reset to 0 (mode ${settings.kizon_mode || 20})`);
    return;
  }

  settings.kizon_consecutive_losses = (settings.kizon_consecutive_losses || 0) + 1;
  logging.info(`KIZON: loss detected; consecutive_losses=${settings.kizon_consecutive_losses}`);

  if (settings.kizon_consecutive_losses < 3) return;

  const candidates = [11, 6, 20, 16, 4];
  const currentMode = Number(settings.kizon_mode || 20);
  const alternatives = candidates.filter(mode => mode !== currentMode);
  const nextMode = alternatives[Math.floor(Math.random() * alternatives.length)] || 20;

  settings.kizon_mode = nextMode;
  settings.kizon_consecutive_losses = 0;
  saveUserSettings();
  logging.warning(`KIZON auto-fallback: 3 consecutive losses; mode ${currentMode} -> mode ${nextMode}`);
}
...
function getTraderFoxPrediction(userId) {
  const results = (userLastResults[userId] || []).slice(-5).map(n => numberToBS(parseInt(n, 10)));
  if (!results.length) return { choice: 'B', shouldSkip: false };

  const last = results[results.length - 1];
  const opposite = last === 'B' ? 'S' : 'B';
  let streak = 1;
  for (let i = results.length - 2; i >= 0 && results[i] === last; i--) streak++;

  // Fox waits for an overextended run, then hunts the reversal; otherwise it
  // follows the freshest short-term direction.
  const choice = streak >= 3 ? opposite : last;
  logging.info(`TRADER_FOX: recent=${results.join('')}, streak=${streak}, prediction=${choice}`);
  return { choice, shouldSkip: false };
}
        ...
        if (userPendingBets[userId]) {
          for (const [period, betInfo] of Object.entries(userPendingBets[userId])) {
            ...
            if (settled && settled.number) {
              ...
              autoFallbackKizon(settings, isWin);
...
async function bettingWorker(userId, ctx, bot) {
  ...
  settings.consecutive_losses = 0;
  if (settings.strategy === "KIZON") {
    settings.kizon_mode = Number.isInteger(Number(settings.kizon_mode)) && Number(settings.kizon_mode) >= 1 && Number(settings.kizon_mode) <= 20
      ? Number(settings.kizon_mode) : 20;
    settings.kizon_consecutive_losses = 0;
  }
  ...
  let strategyText = settings.strategy === "TREND_FOLLOW" ? "Trend Follow" :
                     ...
                     settings.strategy === "KIZON" ? `KIZON (Mode ${settings.kizon_mode || 20})` :
                     settings.strategy === "RISK_AURA" ? "RISK AURA" :
                     settings.strategy === "RISK_AURA_V2" ? "RISK AURA V2" :
                     settings.strategy === "RISK_AGENT" ? "RISK AGENT" :
                     settings.strategy === "KIZON_V2" ? "KIZON V2" :
  ...
  try {
    while (settings.running) {
      ...
      if (["R_PAT", "CHI_SQUARE_ANOMALY_DETECTION", "MULTIPLICATION_MATRIX", "KIZON", "KIZON_V2"].includes(settings.strategy)) {
      ...
      if (settings.strategy === "KIZON") {
        const prediction = getKizonPrediction(strategyOutcomes, settings.kizon_mode || 20);
        ch = prediction.choice;
        shouldSkip = prediction.shouldSkip;
        if (shouldSkip) skipReason = `(${prediction.skipReason || 'KIZON'})`;
      } else if (settings.strategy === "RISK_AURA") {
        const riskAuraOutcomes = await getRiskAuraRecent10(session);
        const prediction = getRiskAuraPrediction(riskAuraOutcomes, userLastResults[userId] || []);
        ch = prediction.choice;
        shouldSkip = prediction.shouldSkip;
        logging.info(`RISK_AURA: choice=${ch}, shouldSkip=${shouldSkip}, source=${riskAuraOutcomes.length ? 'TRX-API-10' : 'LOCAL-10'}`);
        if (shouldSkip) skipReason = `(${prediction.skipReason || 'RISK AURA'})`;
      } else if (settings.strategy === "RISK_AURA_V2") {
        const riskAuraOutcomes = await getRiskAuraRecent10(session);
        const prediction = getRiskAuraV2Prediction(riskAuraOutcomes, userLastResults[userId] || []);
        ch = prediction.choice;
        shouldSkip = prediction.shouldSkip;
        logging.info(`RISK_AURA_V2: choice=${ch}, shouldSkip=${shouldSkip}, source=${riskAuraOutcomes.length ? 'TRX-API-10' : 'LOCAL-10'}`);
        if (shouldSkip) skipReason = `(${prediction.skipReason || 'RISK AURA V2'})`;
      } else if (settings.strategy === "RISK_AGENT") {
        const riskAgentOutcomes = await getRiskAuraRecent10(session);
        const prediction = getRiskAgentPrediction(riskAgentOutcomes, userLastResults[userId] || []);
        ch = prediction.choice;
        shouldSkip = prediction.shouldSkip;
        logging.info(`RISK_AGENT: choice=${ch}, signal=${prediction.signal}, confidence=${prediction.confidence}%, source=${riskAgentOutcomes.length ? 'TRX-API-10' : 'LOCAL-10'}`);
        if (shouldSkip) skipReason = `(${prediction.skipReason || 'RISK AGENT'})`;
      } else if (settings.strategy === "KIZON_V2") {
        const prediction = getKizonV2Prediction(strategyOutcomes);
        ch = prediction.choice;
        shouldSkip = prediction.shouldSkip;
        if (shouldSkip) skipReason = `(${prediction.skipReason || 'KIZON V2'})`;
      } else if (settings.strategy === "CHI_SQUARE_ANOMALY_DETECTION") {
        ...
        ch = prediction.choice;
        shouldSkip = prediction.shouldSkip;
      } else if (settings.strategy === "TRADER_FOX") {
        const prediction = getTraderFoxPrediction(userId);
        ch = prediction.choice;
        shouldSkip = prediction.shouldSkip;
      ...
      // RISK strategies always place a prediction, even when remote history is unavailable.
      // Their predictors already fall back to the local last-10 results/default signal.
      if (["RISK_AURA", "RISK_AURA_V2", "RISK_AGENT"].includes(settings.strategy)) {
        if (shouldSkip) {
          logging.warning(`${settings.strategy}: overriding skip (${skipReason || 'unspecified'}) and placing the fallback prediction`);
        }
        shouldSkip = false;
        skipReason = "";
      }
      ...
      if (shouldSkip) {
        logging.warning(`${settings.strategy}: SKIPPING issue=${currentIssue}, reason=${skipReason || 'unspecified'}, entryLayer=${entryLayer}, slLayer=${settings.sl_layer || 0}`);
        let betChoiceText;
        if (betType === "COLOR") {
...
function makeStrategyKeyboard(userId = null) {
  ...
  const keyboard = [
    ...
    [
      Markup.button.callback(`🦊 Trader Fox`, "strategy:TRADER_FOX")
    ],
    [
    ...
    [
      Markup.button.callback(`🧠 KIZON`, "strategy:KIZON")
    ],
    [
      Markup.button.callback(`🌌 RISK AURA`, "strategy:RISK_AURA"),
      Markup.button.callback(`🌌 RISK AURA V2`, "strategy:RISK_AURA_V2"),
      Markup.button.callback(`🤖 RISK AGENT`, "strategy:RISK_AGENT"),
      Markup.button.callback(`🧠 KIZON V2`, "strategy:KIZON_V2")
    ],
    [
...
async function callbackQueryHandler(ctx) {
  ...
  if (data.startsWith("restart_bot:")) {
    ...
    if (settings.strategy === "KIZON" && !settings.kizon_mode) {
      settings.kizon_mode = 20;
    }
  ...
  if (data.startsWith("strategy:")) {
    ...
    if (strategy === "KIZON") {
      userState[userId] = { state: "INPUT_KIZON_MODE" };
      await sendMessageWithRetry(ctx,
        `${EMOJI.SUCCESS} ${STYLE.BOLD('KIZON Strategy')}` +
        `

${EMOJI.INFO} Choose mode 1-20 (default: 20).` +
        `
${STYLE.CODE('Mode 1: Fibonacci Momentum Matrix')}` +
        `
${STYLE.CODE('Mode 20: Vortex Prime Scalper')}` +
        `

${EMOJI.INFO} Enter a mode number:`,
        makeMainKeyboard(true, userId === ADMIN_ID));
      ...
      await sendMessageWithRetry(ctx, `${EMOJI.SUCCESS} ${STYLE.BOLD(`${strategyLabel} Activated`)}`,
        makeMainKeyboard(true, userId === ADMIN_ID));
    } else if (["RISK_AURA", "RISK_AURA_V2", "RISK_AGENT", "KIZON_V2"].includes(strategy)) {
      const strategyLabel = strategy === "RISK_AURA"
        ? "RISK AURA"
        : strategy === "RISK_AURA_V2"
        ? "RISK AURA V2"
        : strategy === "RISK_AGENT"
        ? "RISK AGENT"
        : "KIZON V2";
      await sendMessageWithRetry(ctx, `${EMOJI.SUCCESS} ${STYLE.BOLD(`${strategyLabel} Activated`)}`, makeMainKeyboard(true, userId === ADMIN_ID));
    } else if (["DYNAMIC_TREND_FLIP", "MANUS_V1", "MANUS_V2", "TRADER_FOX"].includes(strategy)) {
...
async function textMessageHandler(ctx) {
  ...
  if (currentState === "INPUT_KIZON_MODE") {
    const mode = Number.parseInt(text.trim(), 10);
    if (!Number.isInteger(mode) || mode < 1 || mode > 20) {
      await sendMessageWithRetry(ctx,
        `${EMOJI.ERROR} KIZON mode must be a whole number from 1 to 20.`,
        makeMainKeyboard(true, isAdmin));
      return;
    }
    userSettings[userId].kizon_mode = mode;
    await sendMessageWithRetry(ctx,
      `${EMOJI.SUCCESS} ${STYLE.BOLD('KIZON mode set:')} ${mode}`,
      makeMainKeyboard(true, isAdmin));
    delete userState[userId];
    saveUserSettings();
    return;
